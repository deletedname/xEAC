...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:eac="urn:isbn:1-931666-33-4"
    ...
    <xsl:template match="eac:eac-cpf">
        <html lang="en">
            ...
            <head>
                ...
                <!-- mapping -->
                <!-- suppressing map for now IL 20160908
    ...
    <xsl:template name="doc-header">
        ...
        <div class="row" typeof="skos:Concept" about="{$id}#concept">
            ...
            <div class="col-md-12">
                ...
                <!-- 20160822 IL changed "Toggle Names " to "Show/Hide All Names", default on (display:none to display:all); glyphicon-triangle-right to glyphicon-triangle-bottom
                 REASON: to make a default display of all the names with option to see less -->
                
                <div>
                    <h4>
                        <a href="#" id="toggle-names" class="toggle-button">Show/Hide All Names <span
                            class="glyphicon glyphicon-triangle-bottom"/></a>
                    </h4>
                    <div id="names-div" style="display:all">
                        <xsl:if
                        ...
                        <xsl:for-each select="//eac:conventionDeclaration">
                            ...
                            <!--commented out following line convention declarations - IL 20160803
                             don't need to publish this here, was too big and just want to see a list of names, not citations for standards -->
                            <!--<h4>
                                ...
                                <xsl:for-each select="//eac:nameEntry[child::node()=$abbreviation]">
                                    <dt>
                                        <xsl:choose>
                                            ...
                                            <!-- changed display text from "alternative form" to "uncontrolled form" - IL 20160808 -->
                                            
                                            <xsl:when test="eac:alternativeForm">
                                                <xsl:text>uncontrolled form</xsl:text>
                                    ...
                                    <dd>
                                        ...
                                        <!-- dates of use on new line instead of after a comma. IL 20160808-->
                                        ...
                                        <xsl:if test="eac:useDates">
                                            <br>
                                                ...
                                                <xsl:choose>
                                                    ...
                                                    <xsl:when test="eac:useDates/eac:dateSet">
                                                        <xsl:value-of select="string-join(eac:useDates/eac:dateSet/eac:date, ', ')"/>
                                                    </xsl:when>
                                                </xsl:choose>
                                            </br>
                                        </xsl:if>
                                        ...
                                        <!-- adding convention name only when for authorized forms in new paragraph. IL 20160808
                                         
                                         LOVE TO ADD: EXTERNAL LINK TO LC URI, but I'm not sure how to code it because the URI is cited in sources. -->
        ...
        <!-- maintenance history snapshot IL 20160912-->
        <p/>
        <strong>Written by: </strong>
        <xsl:value-of select="//eac:maintenanceEvent[1]/eac:agent"/>
        <br>
        <strong>Last modified: </strong>
        </br>
        <xsl:value-of select="//eac:maintenanceEvent[last()]/eac:eventDateTime"/>
    <hr/>
    ...
    <xsl:template name="cpfDescription-vis-structure">
        <div class="row" typeof="{$class}" about="{$id}">
            <div
                class="col-md-12">
            <!-- supressing map for now IL 20160908
    ...
    <xsl:template match="eac:description">
        <div id="description" class="eac-section">
            ...
            <!-- Added new listed section, SOURCES IL 20190829
             WOULD LOVE TO CREATE HANGING INDENT -->
            ...
            <!-- removed occupation dates from chronology
             or parent::eac:occupation
             IL 20160913 OH YEAH -->
            
            <xsl:if
                test="descendant::eac:date[not(parent::eac:existDates or parent::eac:occupation)] or descendant::eac:dateRange[not(parent::eac:existDates or parent::eac:occupation)]">
                <div id="chron">
                    
                    <h3>Chronology</h3>
                    <ul>
                        <xsl:apply-templates
                            select="descendant::eac:date[@standardDate][not(parent::eac:existDates or parent::eac:occupation)]|descendant::eac:dateRange[eac:fromDate[@standardDate]][not(parent::eac:existDates or parent::eac:occupation)]"
                            mode="chronList">
                            <xsl:sort
            ...
            <!-- removing the command [not(descendant::*/@standardDate)] from eac:place (and all terms) in xsl:if statement IL 20160823
             REASON: places with dates were not displaying under Terms
             RESULT: this added ALL the place names to the Terms list, but they are still also being listed in the chronology -->
            
            <xsl:if
                test="descendant::eac:function|descendant::eac:languageUsed|descendant::eac:legalStatus|descendant::eac:localDescription|descendant::eac:mandate|descendant::eac:occupation|descendant::eac:place">
                <div id="terms">
                    <h3>Terms</h3>
                    <dl class="dl-horizontal">
                        <xsl:apply-templates
                            select="descendant::eac:function|descendant::eac:languageUsed|descendant::eac:legalStatus|descendant::eac:localDescription|descendant::eac:mandate|descendant::eac:occupation|descendant::eac:place">
    ...
    <xsl:template name="chron-description">
        <xsl:choose>
           ...
           <!-- <xsl:when test="parent::node()/eac:term">
            ...
            <!-- xpath modification to ancestor instead of parent
             IL 20160912 -->
            <xsl:when test="ancestor::node()/eac:placeEntry">
                <a
                    ...
                    <xsl:value-of select="ancestor::node()/eac:placeEntry"/>
                </a>
                ...
                <!--  changed placeRole variable into xsl:if instead of xsl:when, original commented out above
                 added descriptive notes to chronlist  IL 20160912 -->
                <xsl:if test="ancestor::node()/eac:placeRole">
                    <span class="subtle">
                    <xsl:text> (</xsl:text>
                    <xsl:value-of select="ancestor::node()/eac:placeRole"/>
                    <xsl:text>) </xsl:text>
                    </span>
                </xsl:if>
                <xsl:if test="ancestor::node()/eac:descriptiveNote">
                    <xsl:text> </xsl:text>
                    <xsl:value-of select="ancestor::node()/eac:descriptiveNote/eac:p"/>
                </xsl:if>
            ...
            <xsl:if test="count(eac:relations/eac:cpfRelation) &gt; 0">
                ...
                <dl class="dl-horizontal">
                    <xsl:apply-templates select="eac:relations/eac:cpfRelation">
                        <!-- sort by corporate body, then by person
                        <xsl:sort select="@xlink:arcrole"/>
                        <xsl:sort select="eac:relationEntry"/> -->
                        <xsl:sort select="@xlink:role" order="descending"/>
                        <xsl:sort select="eac:relationEntry"/>
                    </xsl:apply-templates>
                </dl>
            </xsl:if>
        ...
        <dd>
            <xsl:choose>
                <xsl:when test="local-name()='cpfRelation'">
                    ...
                    <!-- added dates and descriptiveNote to cpfRelations IL 20160906
                     nl=newline currently adding a space instead of carriage return IL 20160913 -->
                    <xsl:variable name="nl" select="'&#10;'"/>
                    <xsl:if test="parent::node()/eac:date|eac:dateRange|eac:dateSet">
                        <span class="subtle">
                        <br>
                            <xsl:text>associated dates: </xsl:text>
                            <xsl:choose>
                                ...
                                <xsl:when test="eac:dateSet">
                                    <xsl:value-of select="string-join(eac:dateSet/eac:date, ', ')"/>
                                </xsl:when>
                            </xsl:choose>
                        </br>
                        </span>
                    </xsl:if>
                    <xsl:if test="string(eac:descriptiveNote)">
                        <br>
                            <xsl:value-of select="string-join(eac:descriptiveNote/eac:p,$nl)"/>
                        </br>
                    </xsl:if>
                ...
                <xsl:when test="local-name()='resourceRelation'">
                    ...
                    <a href="{@xlink:href}">
                        ...
                        <!-- bolded resource relation entry IL 20160906 -->
                        <strong>
                        <xsl:value-of select="eac:relationEntry"/>
                        </strong>
                    </a>
                    
                    <!-- added dates and descriptiveNote to resourceRelation IL 20160906
                     date ranges within date sets do not display 
                     nl=newline currently adding a space instead of carriage return IL 20160913 -->
                    <xsl:variable name="nl" select="'&#10;'"/>
                    <xsl:if test="parent::node()/eac:date|eac:dateRange|eac:dateSet">
                        <span class="subtle">
                        <br>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="eac:dateSet/eac:dateRange">
                                    <xsl:value-of
                                    select="string-join(('eac:dateRange/eac:fromDate', 'eac:dateRange/eac:toDate'), '-')"/>
                                </xsl:when>
                            </xsl:choose>
                        </br>
                        </span>
